...
import 'package:fahem_business/data/models/static/appointment_booking_model.dart';
...
import 'package:fahem_business/presentation/btm_sheets/governorates_btm_sheet.dart';
...
import 'package:fahem_business/data/models/static/governorate_model.dart';
import 'package:fahem_business/data/models/account_model.dart';

class EditAccountProfileProvider with ChangeNotifier {
  ...
  Future<bool> uploadImages(BuildContext context) async {
    ...
    for(int i=0; i<photoGallery.length; i++) {
      if(isUploadFileError == false) {
        if(photoGallery[i].startsWith(ConstantsManager.fahemBusinessImageFromFile)) {
    ...
    for(int i=0; i<identificationImages.length; i++) {
      if(isUploadFileError == false) {
        if(identificationImages[i].startsWith(ConstantsManager.fahemBusinessImageFromFile)) {
          UploadFileParameters uploadFileParameters = UploadFileParameters(
  ...
  toggleSelectedCategories(CategoryModel selectedCategory) {
  ...
  // region Edit Account Profile
  Future<void> editAccountProfile({required BuildContext context, required EditAccountProfileParameters parameters}) async {
    changeIsLoading(true);
    bool isUploadFileError = await uploadImages(context);
    if(isUploadFileError == false) {
      parameters.photoGallery = _photoGalleryRandomName;
      parameters.identificationImages = _identificationImagesRandomName;
      ...
      if(_nationalImageFrontSide is XFile) {
        ...
        parameters.nationalImageFrontSide = imageName;
      }
      if(_nationalImageBackSide is XFile) {
        ...
        parameters.nationalImageBackSide = imageName;
      }
      if(_cardImage is XFile) {
        ...
        parameters.cardImage = imageName;
      }
      Either<Failure, AccountModel> response = await DependencyInjection.editAccountProfileUseCase.call(parameters);
      await response.fold((failure) async {
        ...
        MyProviders.authenticationProvider.changeCurrentAccount(account);
  ...
  Future<void> fetchServices({required int mainCategoryId}) async {
    ...
    filtersMap.addAll({'serviceProviderCanSubscribe': true});
