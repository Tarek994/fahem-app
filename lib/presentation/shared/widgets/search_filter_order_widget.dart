...
FilterModel getFilterModel(List<FilterModel> filters, Filters filter) {
  FilterModel filterModel =
      filters.firstWhere((element) => element.filter == filter);
  return filterModel;
}
...
Map<String, dynamic> getResults(List<FilterModel> filters) {
  ...
  if (isExistInFilters(filters, Filters.commentStatus)) {
    CommentStatus commentStatus =
        (getFilterModel(filters, Filters.commentStatus).value);
  ...
  if (isExistInFilters(filters, Filters.walletTransactionType)) {
    WalletTransactionType walletTransactionType =
        (getFilterModel(filters, Filters.walletTransactionType).value);
    results.addAll(
        {Filters.walletTransactionType.name: walletTransactionType.name});
  }
  if (isExistInFilters(filters, Filters.withdrawalRequestStatus)) {
    WithdrawalRequestStatus withdrawalRequestStatus =
        (getFilterModel(filters, Filters.withdrawalRequestStatus).value);
    results.addAll(
        {Filters.withdrawalRequestStatus.name: withdrawalRequestStatus.name});
  }
  if (isExistInFilters(filters, Filters.paymentType)) {
    PaymentType paymentType =
        (getFilterModel(filters, Filters.paymentType).value);
    results.addAll({Filters.paymentType.name: paymentType.name});
  }
  if (isExistInFilters(filters, Filters.startDateOfCreated)) {
    DateTime dateTime =
        (getFilterModel(filters, Filters.startDateOfCreated).value);
    results.addAll(
        {Filters.startDateOfCreated.name: dateTime.millisecondsSinceEpoch});
  }
  if (isExistInFilters(filters, Filters.endDateOfCreated)) {
    DateTime dateTime =
        (getFilterModel(filters, Filters.endDateOfCreated).value);
    results.addAll({
      Filters.endDateOfCreated.name:
          dateTime.add(const Duration(days: 1)).millisecondsSinceEpoch - 1000
    });
  }
  if (isExistInFilters(filters, Filters.startPeriodDate)) {
    DateTime dateTime =
        (getFilterModel(filters, Filters.startPeriodDate).value);
    results.addAll(
        {Filters.startPeriodDate.name: dateTime.millisecondsSinceEpoch});
  }
  if (isExistInFilters(filters, Filters.endPeriodDate)) {
    DateTime dateTime = (getFilterModel(filters, Filters.endPeriodDate).value);
    results.addAll({
      Filters.endPeriodDate.name:
          dateTime.add(const Duration(days: 1)).millisecondsSinceEpoch - 1000
    });
  }
  ...
  if (isExistInFilters(filters, Filters.mainCategory)) {
    MainCategoryModel mainCategory =
        (getFilterModel(filters, Filters.mainCategory).value);
  ...
  if (isExistInFilters(filters, Filters.instantConsultation)) {
    InstantConsultationModel instantConsultation =
        (getFilterModel(filters, Filters.instantConsultation).value);
    results.addAll({
      Filters.instantConsultation.name:
...
class SearchFilterOrderWidget extends StatefulWidget {
  ...
  @override
  State<SearchFilterOrderWidget> createState() =>
      _SearchFilterOrderWidgetState();
}
      ...
      child: SizedBox(
        ...
        child: Row(
          ...
          children: [
              ...
              Expanded(
                child: CustomTextFormField(
                  ...
                  hintText: widget.hintText == null
                      ? null
                      : Methods.getText(widget.hintText!).toCapitalized(),
                  ...
                  suffixIcon: Container(
                    ...
                    decoration: BoxDecoration(
                      border: Border.all(color: ColorsManager.veryDarkBlue),
                      borderRadius: BorderRadius.circular(14),
                    ),
                    child: InkWell(
                      borderRadius: BorderRadius.circular(14),
                      onTap: widget.dataState == DataState.loading
                          ? null
                          : () async {
                              ...
                              await Dialogs.showBottomSheet(
                                ...
                                Map<String, dynamic>
                                    currentFiltersSelectionResults =
                                    getResults(_currentFiltersSelection);
                                Map<String, dynamic> filtersResultDataResults =
                                    getResults(FiltersResult.filters);
                                debugPrint(filtersResultDataResults.toString());
                                if (currentFiltersSelectionResults.toString() ==
                                    filtersResultDataResults.toString()) return;
                                _currentFiltersSelection =
                                    FiltersResult.filters;
                                globalFilters =
                                    jsonEncode(filtersResultDataResults);
                                widget.reFetchData();
                              });
                            },
                      child: Row(
                        ...
                        children: [
                          ...
                          Icon(Icons.filter_list,
                              color: ColorsManager.veryDarkBlue, size: 15),
                          SizedBox(width: 4),
                          Text(
                            Methods.getText(StringsManager.filters)
                                .toCapitalized(),
                            style: TextStyle(
                                fontSize: 12,
                          ...
                          Icon(Icons.arrow_drop_down,
                              color: ColorsManager.veryDarkBlue, size: 13),
                        ],
                      ),
                    ),
                  ),
            ...
            //if (widget.isSupportOrder)
            // IconButton(
            //   onPressed: widget.dataState == DataState.loading
            //       ? null
            //       : () async {
            //           FocusScope.of(context).unfocus();
            ...
            //             ),
            //           ).then((_) async {
            //             Map<String, dynamic> currentFiltersSelectionResults =
            //                 getResults(_currentFiltersSelection);
            //             Map<String, dynamic> filtersResultDataResults =
            //                 getResults(FiltersResult.filters);
            //             debugPrint(filtersResultDataResults.toString());
            ...
            //           size: SizeManager.s20,
            //           color: ColorsManager.lightPrimaryColor),
            //       if (_currentFiltersSelection.isNotEmpty)
            //         const PositionedDirectional(
            //           top: -3,
            //           start: -3,
            //           child: CircleAvatar(
            //               radius: SizeManager.s5,
