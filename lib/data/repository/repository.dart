...
class Repository extends BaseRepository {
  ...
  @override
  Future<Either<Failure, UserModel?>> checkUserExistWithPhoneNumberAndGet(CheckUserExistWithPhoneNumberAndGetParameters parameters) async {
    if(await _baseNetworkInfo.isConnected) {
      try {
        var result = await _baseRemoteDataSource.checkUserExistWithPhoneNumberAndGet(parameters);
        debugPrint('checkUserExistWithPhoneNumberAndGet');
        return Right(result);
      }
      on ServerException catch(error) {
        return Left(ServerFailure(messageAr: error.messageAr, messageEn: error.messageEn));
  ...
  @override
  Future<Either<Failure, List<StatisticModel>>> getAccountStatistics(GetAccountStatisticsParameters parameters) async {
    if(await _baseNetworkInfo.isConnected) {
      try {
        var result = await _baseRemoteDataSource.getAccountStatistics(parameters);
        debugPrint('getAccountStatistics');
        return Right(result);
      }
      on ServerException catch(error) {
        return Left(ServerFailure(messageAr: error.messageAr, messageEn: error.messageEn));
